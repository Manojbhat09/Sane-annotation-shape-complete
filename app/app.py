...
from rl_gan.test_rl_module import test_sample
import glob
...
@app.route("/getRLGANPoints", methods=['POST'])
def getRLGANPoints():
	json_request = request.get_json()
	fname = json_request["fname"]
	drivename, fname = fname.split("/")
	bboxes = json_request["bounding_boxes"]
	print(bboxes)
	print("Testing ...")
	if bboxes:
		pcds = test_sample(drivename, fname, bboxes)
		print("done testing")
		print(drivename)
		print(fname)
		#print(pred_list)
		...
		for pcd in pcds:
			if len(all_points) == 0:
				all_points = pcd
			all_points = np.vstack((all_points,pcd))
			car_count+=1
		all_points = np.array(all_points)
		all_points = all_points.reshape(-1)
		print(len(all_points))
		all_points = all_points.tolist()
		# import pdb; pdb.set_trace()
		return ','.join(map(str,all_points))
	else:
		print("Something wrong, no boxes found")

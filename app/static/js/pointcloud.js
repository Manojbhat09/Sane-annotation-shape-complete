...
function generatePointCloudSubset( vertices, color, DATA_STRIDE, name, add_mesh) {
    var geometry = new THREE.Geometry();
    var colors = [];
    var k = 0;
    var __vertices = [];
    ...
    // geometry.colors = colors;
    geometry.computeBoundingBox();
    var material = new THREE.PointsMaterial( { size: 3, sizeAttenuation: false, vertexColors: THREE.VertexColors } );
    // creates pointcloud given vectors
    var pointcloud_subset = new THREE.Points( geometry, material );
    pointcloud_subset.name = name;
    scene.add(pointcloud_subset);
    // greet("hello");
    if(add_mesh){
        var geometry_mesh = new Convex( geometry.vertices );
        var material_mesh = new THREE.MeshBasicMaterial( {color: 0x00ff00, opacity: 0.6,transparent: true} );
        // var material_mesh = new THREE.MeshLambertMaterial( {
        //     color: 0x00ff00,
        //     opacity: 0.8,
        //     transparent: true
        // } );
        var mesh = new THREE.Mesh( geometry_mesh, material_mesh );
        mesh.material.side = THREE.BackSide; // back faces
        mesh.renderOrder = 0;
        mesh.name = "convex_mesh"
        console.log("Adding mesh")
        scene.add( mesh );

    }

    return pointcloud_subset;
}    

function deletePointCloud(name, points_delete=true,mesh_delete=true){
    const object_delete = scene.getObjectByName( name );
    if(object_delete && points_delete){
        object_delete.geometry.dispose();
        object_delete.material.dispose();
        scene.remove( object_delete );
    }
    const object_delete_mesh = scene.getObjectByName(  "convex_mesh" );
    if(object_delete_mesh && mesh_delete){
        object_delete_mesh.geometry.dispose();
        object_delete_mesh.material.dispose();
        scene.remove( object_delete_mesh );
    }
}
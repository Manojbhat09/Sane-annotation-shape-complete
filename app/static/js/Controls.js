...
var SettingsControls = function() {
    ...
    this.ShapeCompletion = false;
    this.PointCompletion = false;
    this.speed = 0.8;
...
settingsFolder.add(settingsControls, 'ShapeCompletion').onChange(function() {
    if(settingsControls["ShapeCompletion"] == true){
        console.log("showing")
        deletePointCloud("RLGAN_points")
        generatePointCloudSubset(app.completion_data, COLOR_RED, 3, "RLGAN_points", true);
        // settingsControls["PointCompletion"] = false;
    }
    else{
        console.log("Deleting completed points")
        deletePointCloud("RLGAN_points", points_delete=false, mesh_delete=true)
    }
});

settingsFolder.add(settingsControls, 'PointCompletion').onChange(function() {
    if(settingsControls["PointCompletion"] == true){
        console.log("change in pcn")
        app.push_box_frames(app.cur_frame.fname);
